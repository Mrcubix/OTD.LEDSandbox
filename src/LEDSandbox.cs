...
namespace OTD.LEDSandbox
{
    ...
    public class LEDSandbox : ITool
    {
            ...
            if (Info.Driver is Driver driver)
            {
                ...
                if (device.VendorID == SUPPORTED_VENDORID && SupportedProductID.Contains(device.ProductID))
                {
                    ...
                    if (topDisplayImage != null)
                        ProcessImage(topDisplayImage, 0, FlipTopDisplayImage, WIDTH, HEIGHT, reader.ReportStream);

                    if (bottomDisplayImage != null)
                        ProcessImage(bottomDisplayImage, 4, FlipBottomDisplayImage, WIDTH, HEIGHT, reader.ReportStream);
                }
                else
                {
        ...
        private void ProcessImage(FileInfo file, int displayChunk, bool doFlip, int width, int height, HidStream hidStream)
        {
            // Read the image
